...
def solution(request):
    our_solution = models.Solution.objects.all()
...
def count_cart_solution(request):    
    ...
    cart = get_object_or_404(models.Cart, user_id=user_id)
    cart_id = cart.id
    if not cart_id:
        request.session['items_total'] = 0   
    else:
        request.session['items_total'] = models.CartItem.objects.filter(cart=cart_id).count()   


def add_to_cart(request, solution_id):
    solution = get_object_or_404(models.Solution, id=solution_id)
    user_id = request.user.id
    usercarts = models.Cart.objects.filter(user_id=user_id).all()
    if not usercarts:
        cart = models.Cart.objects.create(user=request.user)
    else:
        cart = get_object_or_404(models.Cart, user_id=user_id)
    cart_item, created = models.CartItem.objects.get_or_create(cart=cart, solution=solution)
    if not created:
...
def remove_from_cart(request, solution_id):
    user_id = request.user.id
    cart = get_object_or_404(models.Cart, user_id=user_id)
    cart_item = get_object_or_404(models.CartItem, cart=cart, solution_id = solution_id)
    cart_item.delete()
    count_cart_solution(request)
    return redirect('solution')

def order_create(request):
    cart = get_object_or_404(models.Cart, user_id=request.user.id)
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                for item in cart.cartitem_set.all():
                    solution = get_object_or_404(models.Solution, id = item.solution_id)
